...
class SummarizationModel(object):
  ...
  def _add_placeholders(self):
    ...
    self._enc_number_sents = tf.placeholder(tf.float32, [hps.batch_size, None], name='enc_number_sent')
    self._enc_tf_lists = tf.placeholder(tf.float32, [hps.batch_size, None], name='enc_tf_list')
  ...
  def _make_feed_dict(self, batch, just_enc=False):
    ...
    # add number_sent by viet nguyen
    feed_dict[self._enc_number_sents] = batch.enc_number_sents
    feed_dict[self._enc_tf_lists] = batch.enc_tf_lists
  ...
  def _add_decoder(self, inputs):
    ...
    outputs, out_state, attn_dists, p_gens, coverage = attention_decoder(inputs, self._dec_in_state, self._enc_states, self._enc_padding_mask,self._enc_number_sents,self._enc_tf_lists,  cell, initial_state_attention=(hps.mode=="decode"), pointer_gen=hps.pointer_gen, use_coverage=hps.coverage, prev_coverage=prev_coverage)
  ...
  def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, prev_coverage):
    ...
    feed = {
        ...
        self._enc_number_sents: batch.enc_number_sents,
        self._enc_tf_lists: batch.enc_tf_lists,
