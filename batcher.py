...
class Example(object):
  ...
  def pad_encoder_input(self, max_len, pad_id):
    ...
    while len(self.enc_tf_list) < max_len:
      self.enc_tf_list.append(pad_id)
    while len(self.enc_number_sent) < max_len:
      self.enc_number_sent.append(pad_id)
...
class Batch(object):
  ...
  def __init__(self, example_list, hps, vocab):
    ...
    self.init_encoder_seq(example_list, hps) # initialize the input to the encoder... khoi tao dau vao cho bo ma hoa
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    # Xac dinh do dai toi da cua bo ma hoa trong batch nay
    max_enc_seq_len = max([ex.enc_len for ex in example_list])

    # Pad the encoder input sequences up to the length of the longest sequence
    # Dua tat ca cac cau ve do dai cau dai nhat
    ...
    self.enc_number_sents = np.zeros((hps.batch_size, max_enc_seq_len), dtype=np.float32)
    self.enc_tf_lists = np.zeros((hps.batch_size, max_enc_seq_len), dtype=np.float32)
    ...
    for i, ex in enumerate(example_list):
      ...
      self.enc_number_sents[i,:] = ex.enc_number_sent[:]
      self.enc_tf_lists[i,:] = ex.enc_tf_list[:]
